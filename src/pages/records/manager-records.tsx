...
const AllManagerEntriesPage: FC = function () {
    ...
    const getDateRecordsByDateRange = async () => {
        const res = await api.get(`/manager/date-by-date-range/?from=${selectionRange.startDate}&to=${selectionRange.endDate}`);
        if (res.data.status === 'success') {
            setDateRecords(res.data.data);
        }
        else {
                    ...
                    <div className="block items-center sm:flex">
                        <div className="flex flex-col gap-2 relative">
                            ...
                            <TextInput
                                ...
                                className="mt-1 w-96" />
                            {showDateRangeFilter && <DateRangePicker ranges={[selectionRange]} onChange={(e: any) => { setSelectionRange({ ...selectionRange, startDate: e.selection.startDate, endDate: e.selection.endDate }); setShowDateRangeFilter(false) }} className="absolute !z-[100px] top-20 shadow-3xl border-black border" />}
                        </div>
                            ...
                            <div className="mb-4 h-full rounded-lg bg-white p-4 shadow dark:bg-gray-800 sm:p-6">
                                ...
                                <div className="flow-root">
                                    <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                                        {dateRecords.length > 0 && dateRecords.map((dateRecord: any) =>
                                            <li className="py-3 sm:py-4">
                                                <div className="flex items-center space-x-4">


                                                    {
                                                        dateRecord?.total_pending_entries > 0 ? <div style={{ width: "10px", height: "10px", padding: "1px", backgroundColor: "red", borderRadius: "50px" }} ></div> : <div style={{ width: "10px", height: "10px", padding: "1px", backgroundColor: "green", borderRadius: "50px" }} ></div>
                                                    }
                                                    ...
                                                    <div className="inline-flex items-center gap-3 text-base font-semibold text-gray-900 dark:text-white">
                                                        <Button color="success" onClick={() => navigate(`/managerEntries-by-date/${dateFormat(dateRecord.date, "dd-mmm-yyyy")}`)}> Go to Entries</Button>
                                                        <EditManagerDateModal dateRecord={dateRecord} dateRecordId={dateRecord.id} getDateRecords={getDateRecords} />
                                                        <DeleteManagerDateModal dateRecordId={dateRecord.id} getDateRecords={getDateRecords} />
                                                    </div>
                                                </div>
                                            </li>
                                        )}
                                        {dateRecords.length == 0 && <li className=" h-96 text-center">No Record Found</li>}
                                    </ul>
                                </div>
