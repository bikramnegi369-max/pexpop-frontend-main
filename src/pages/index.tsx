...
const DashboardPage: FC = function () {
  ...
  const totalProfit =
    dateRecords.length > 0
      ? dateRecords.reduce(
          (total, dateRecord) => total + dateRecord.total_profit,
          0
        )
      : 0;
  const totalAgentCommission =
    dateRecords.length > 0
      ? dateRecords.reduce(
          (total, dateRecord) => total + dateRecord.total_agent_commission,
          0
        )
      : 0;
          ...
          <div className="flex flex-col gap-2">
            ...
            {showDateRangeFilter && (
              <DateRangePicker
                ...
                className="absolute top-48 border border-black shadow-2xl"
              />
            )}
          </div>
        </div>
      </div>
      <div className="mx-[20px] mt-[50px] grid grid-cols-2 gap-4 md:grid-cols-2 md:gap-6 xl:grid-cols-3 2xl:gap-8">
